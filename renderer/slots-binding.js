...
export class SlotsBinding {
    ...
    initCells(document) {
        for (let row = 0; row < NO_ROWS; row++) {
            ...
            for (let column = 0; column < NO_COLUMNS; column++) {
                this.slotCells[row].push(
                    document.getElementById(`${row}-${column}`)
                );
            }
        }

        this.betSelectCells = Array.from(
            document.getElementsByClassName("bet-select-cell")
        );
    }
    ...
    initBinding(document) {
        ...
        this.greyedOutBackground = document.getElementById(
            "greyed-out-background"
        );

        for (let i = 0; i < 5; i++) {
            this.lastChosenColors.push(
                document.getElementById(`last-drawn-card-${i}`)
            );
        }
    }
}
