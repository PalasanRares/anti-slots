...
export class SlotsService {
    ...
    private readonly rustThresholds: {
        rustLevel: RustLevel;
        threshold: number;
    }[] = [
        {
    ...
    private _spinResultSubject: BehaviorSubject<SpinResult | null> =
        new BehaviorSubject<SpinResult | null>(null);
    get spinResultObservable(): Observable<SpinResult> {
        return this._spinResultSubject
            .asObservable()
            .pipe(
                filter((spinResult: SpinResult | null) => spinResult !== null)
            );
    }
    ...
    private _amountLostSubject: BehaviorSubject<number> = new BehaviorSubject(
        0
    );
    ...
    private _spinStateSubject: BehaviorSubject<SpinState> =
        new BehaviorSubject<SpinState>(SpinState.NOT_SPINNING);
    get spinStateObservable(): Observable<SpinState> {
        return this._spinStateSubject.asObservable();
    }

    private _selectedBetSubject: BehaviorSubject<SelectedBet> =
        new BehaviorSubject<SelectedBet>({
            current: 0.2,
            last: 0.2
        });
    ...
    private _currentWinSubject: BehaviorSubject<number> =
        new BehaviorSubject<number>(0);
    get currentWinObservable(): Observable<number> {
        return this._currentWinSubject.asObservable();
    }

    private _randomChosenColorSubject: BehaviorSubject<DoublingColor> =
        new BehaviorSubject<DoublingColor>(DoublingColor.RED);
    get randomChosenColorObservable(): Observable<DoublingColor> {
        return this._randomChosenColorSubject.asObservable();
    }

    private _rustLevelSubject: BehaviorSubject<RustLevel> =
        new BehaviorSubject<RustLevel>(RustLevel.LEVEL_0);
    get rustLevelObservable(): Observable<RustLevel> {
        return this._rustLevelSubject.asObservable();
    }

    public spin() {
        this._amountWonSubject.next(
            this._amountWonSubject.value + this._currentWinSubject.value
        );
        this._amountLostSubject.next(
            this._amountLostSubject.value +
                this._selectedBetSubject.value.current
        );
        const rustLevel = this.rustThresholds.findLast(
            (rustThreshold) =>
                this._amountLostSubject.value >= rustThreshold.threshold
        )!;
    ...
    public selectBet(bet: number) {
        if (
            this._spinStateSubject.value !== SpinState.NOT_SPINNING &&
    ...
    public double(color: DoublingColor) {
        ...
        const randomColor =
            Math.random() < 0.5 ? DoublingColor.RED : DoublingColor.BLACK;
